...
#if defined(NDEBUG)
# error "Nasarium cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb40d896922bb733de785576d9812da110ac076e65c19f3c3976db93f01958c11");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nasarium: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nasarium Signed Message:\n";
...
{
    // Nasarium: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 50 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 44550000 * COIN;
            }

    // Subsidy is cut in half every 825000 blocks
    nSubsidy >>= (nHeight / 825000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nasarium: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nasarium: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1525386633, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Nasarium Project NASAR";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04836bab7781ccc5adbe79786683ce36ad321a5191340daa7377cc95a7415eea60e1d40ec2331cdc23db283da7e08398546a94925d8e6d6cadc02ca92f8d349a09") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1525386633;
        ...
        assert(block.hashMerkleRoot == uint256("0xb97d539186a0f741eaa786cd3c88409098a0d294e914798b2298e5f335c56e6a"));
...
unsigned char pchMessageStart[4] = { 0x0a, 0x07, 0xea, 0xde }; // Nasarium: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NasariumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NasariumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NasariumMiner(CWallet *pwallet)
{
    printf("NasariumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nasarium-miner");
    ...
    try { loop {
        ...
        printf("Running NasariumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NasariumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NasariumMiner, pwallet));
}
